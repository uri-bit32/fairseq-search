...
#        pdb.set_trace()
        ...
        self.ema_decoder = copy.deepcopy(decoder)
...
#                                                           [int(embed_dim / 4), int(embed_dim * 2 / 4), int(embed_dim * 3 / 4),  embed_dim])
    ...
    def forward_embedding(self, src_tokens):
        ...
        x = F.dropout(x, p=self.dropout[self.index[0]], training=self.training)
        return x, embed
...
#        pdb.set_trace()
        ...
        x = self.embedding_hidden_mapping_in(x, index[0])
        encoder_embedding = self.embedding_hidden_mapping_in(encoder_embedding, index[0])
        # B x T x C -> T x B x C
        x = x.transpose(0, 1)
...
#            if not self.training or (dropout_probability > self.encoder_layerdrop):
                x = layer(x, [index[0], index[idx]], encoder_padding_mask)
                idx = idx + 1
...
#        pdb.set_trace()
        ...
        x = F.dropout(x, p=self.dropout[index[0]], training=self.training)
...
#            dropout_probability = torch.empty(1).uniform_()
        ...
        if self.layer_norm is not None:
            x = self.layer_norm(x)
        x = self.embedding_hidden_mapping_out(x, index[0])
        # T x B x C -> B x T x C
        x = x.transpose(0, 1)
...
#                if index==0:
#                   return F.linear(features, self.embed_tokens.weight[:, :int(self.embed_dim / 4)])
#                elif index==1:
#                   return F.linear(features, self.embed_tokens.weight[:, :int(self.embed_dim * 2 / 4)])
#                elif index==2:
#                   return F.linear(features, self.embed_tokens.weight[:, :int(self.embed_dim * 3 / 4)])
#                else:
