...
class EnsembleModel(torch.nn.Module):
    ...
    def forward_encoder(self, encoder_input):
        arc_index = INDEX[0]
        layer_index = INDEX[1:]
        encoder_input["index"] = [arc_index] + layer_index[:6]
        if not self.has_encoder():
            return None
        if EMA_MODEL==0:
           return [model.encoder(**encoder_input) for model in self.models]
        else:
           return [model.ema_encoder(**encoder_input) for model in self.models]
    ...
    def _decode_one(
        ...
        if self.incremental_states is not None:
            ...
            decoder_out = list(model.forward_decoder(
                EMA_MODEL,  [arc_index] + layer_index[6:] , tokens, encoder_out=encoder_out, incremental_state=self.incremental_states[model],
            ))
        else:
