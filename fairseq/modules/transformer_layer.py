...
#        self.final_layer_norm = SlimmableLayernorm([int(self.embed_dim / 4), int(self.embed_dim * 2 / 4), int(self.embed_dim * 3 / 4), self.embed_dim])
    ...
    def forward(self, x, idx, encoder_padding_mask, attn_mask: Optional[Tensor] = None):
        """
        Args:
...
#        pdb.set_trace()
        ...
        x, _ = self.self_attn(
            index=idx, query=x, key=x, value=x, key_padding_mask=encoder_padding_mask
        )
...
def SLinear(in_features_list, out_features_list, bias=True):
    m = SlimmableLinear(in_features_list, out_features_list, bias)
